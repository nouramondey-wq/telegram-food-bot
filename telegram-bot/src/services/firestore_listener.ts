...
interface OrderState {
  status: string;
  notifiedNew: boolean;    // FCM + admin alert sent
  notifiedStatus: boolean; // Status change notification sent
}

export class FirestoreOrderListener {
   ...
   */
  private handleNewOrder(order: any, orderId: string): void {
   ...
   */
  private handleOrderUpdate(order: any, orderId: string): void {
    ...
    if (prevState.status === newStatus) return;
   ...
   */
  private async notifyNewOrder(order: any, orderId: string, state: OrderState): Promise<void> {
    if (state.notifiedNew) return;
