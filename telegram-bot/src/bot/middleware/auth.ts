 ...
 */
export async function ensureCustomer(ctx: BotContext, next: () => Promise<void>) {
  ...
  try {
    ...
    if (snapshot.empty) {
      ...
      const newCustomer = {
        ...
        is_blocked: false,
        notes: '',
        first_interaction_at: admin.firestore.FieldValue.serverTimestamp(),
